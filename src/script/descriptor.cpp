...
protected:
    std::vector<CScript> MakeScripts(const std::vector<CPubKey>& keys, std::span<const CScript> scripts, FlatSigningProvider& out) const override
    {
        ...
        // P2MR only uses the merkle root
        // Subsequently, use NUMS_H as placeholder since P2MR doesn't use keypath
...
public:
    ...
    std::optional<int64_t> MaxSatisfactionWeight(bool) const override {
        // P2MR only supports script path, no keypath
    ...
    std::unique_ptr<DescriptorImpl> Clone() const override
    {
        ...
        return std::make_unique<P2MRDescriptor>(std::move(subdescs), m_depths);
    }
};
...
enum class ParseScriptContext {
    ...
    P2MR,   //!< Inside tmr() (Bip360 script leaf only)
};
...
std::vector<std::unique_ptr<DescriptorImpl>> ParseScript(uint32_t& key_exp_index, std::span<const char>& sp, ParseScriptContext ctx, FlatSigningProvider& out, std::string& error)
{
    ...
    if (ctx == ParseScriptContext::TOP && Func("tmr", expr)) {
        // P2MR only supports script path, no internal key
        ...
        if (expr.size()) {
            ...
            do {
                ...
                while (Const("{", expr)) {
                    branches.push_back(false); // new left branch
                    if (branches.size() > TAPROOT_CONTROL_MAX_NODE_COUNT) {
                        error = strprintf("tmr() supports at most %i nesting levels", TAPROOT_CONTROL_MAX_NODE_COUNT);
                        return {};
                    }
                }
                // Process the actual script expression.
        ...
        // Build the final descriptors vector
        // For tmr(), we create a single descriptor with all subdescriptors
        ...
        ret.emplace_back(std::make_unique<P2MRDescriptor>(std::move(all_descs), depths));
        return ret;
    ...
    // Process miniscript expressions.
    {
        ...
        if (node) {
            if (ctx != ParseScriptContext::P2WSH && ctx != ParseScriptContext::P2TR && ctx != ParseScriptContext::P2MR) {
                error = "Miniscript expressions can only be used in wsh, tr, or tmr.";
                return {};
            }
