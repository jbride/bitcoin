...
enum class script_verify_flag_name : uint8_t {
    ...
    // Placed last so upstream flag ordinals (and bit positions) stay stable.
    SCRIPT_VERIFY_P2MR,
...
private:
    ...
    template<typename U>
    friend bool HandleSLHDSASignature(std::vector<std::vector<unsigned char>>& stack, const CScript& exec_script, script_verify_flags flags, const GenericTransactionSignatureChecker<U>& checker, ScriptExecutionData& execdata, ScriptError* serror);
